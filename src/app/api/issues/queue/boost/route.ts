...
interface BoostIssueBody {
  repo: unknown;
  issueNumber: unknown;
}

export async function POST(request: NextRequest) {
  let parsed: BoostIssueBody;
  try {
    parsed = (await request.json()) as BoostIssueBody;
  } catch {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to boost issue priority";
