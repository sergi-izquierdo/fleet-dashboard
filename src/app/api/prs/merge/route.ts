...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: `PR #${prNumber} queued for squash merge` });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    const isAlreadyMerged =
      message.includes("already been merged") || message.includes("merged");
    return NextResponse.json(
      { error: isAlreadyMerged ? "PR has already been merged" : message },
      { status: isAlreadyMerged ? 409 : 500 }
    );
  }
}
