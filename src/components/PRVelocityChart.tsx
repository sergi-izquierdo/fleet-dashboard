...
function computeRollingAverage(data: PRTrendDay[], window: number): (number | null)[] {
  return data.map((_, i) => {
...
export default function PRVelocityChart() {
  ...
  const fetchTrends = useCallback(async () => {
    try {
      ...
      setTrends(Array.isArray(data) ? (data as PRTrendDay[]) : []);
      setError(null);
    } catch (err) {
  ...
  const avgPerDay =
    trends.length > 0
      ? Math.round((totalMerges / trends.length) * 10) / 10
      : 0;
